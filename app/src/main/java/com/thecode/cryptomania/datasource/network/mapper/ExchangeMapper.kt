...
class ExchangeMapper @Inject constructor() :
    ...
    override fun mapToDomain(entity: ExchangeObjectResponse): Exchange {
        return Exchange(
            entity.exchanges.map {
                mapFromExchangeItem(it)
            }
        )
    }

    private fun mapFromExchangeItem(exchange: ExchangeObjectResponse.Result): ExchangeItem {
        return ExchangeItem(
            exchange.id,
            exchange.name,
